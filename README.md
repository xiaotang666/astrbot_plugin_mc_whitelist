...
转发要**同时**满足：群服互联已启用 → 「启用 QQ→MC 广播」开着 → 消息是普通聊天
（不是 @机器人 / 带唤醒前缀的指令）→ 群在允许范围 → 不在黑名单 →
有「已启用且勾了『转发群消息』」的服务器 → 该服务器链路已连接。

任何一条不满足都会在日志里写明原因（同一条原因 60 秒只提示一次）：

| 日志 | 含义 |
|---|---|
| `[MCWL] QQ→MC 转发：广播=开，触发前缀=（空：全部消息转发），目标=主城服` | 重载插件时自述的转发条件 |
| `[MCWL] 群消息已转发到 主城服（玩家名，12 字）` | 已发出 |
